...
6. At the start of a task, use `focus --query` with distinctive task terms. Open only its capped `evidence_paths` first, and use its correctness gate before finalizing.
7. After a verified investigation, upsert the durable boundary, state transition, test seam, or correction that would make a related future task faster. Add retrieval keywords; never save an unverified conclusion.
8. Run `validate` before finishing.
