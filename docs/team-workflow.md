...
3. Use `node .engram/runtime/bin/engram.js bundle --output engram-team-bundle.json` when handing a repository state to another team or agent. This deliberate JSON artifact contains the Cortex snapshot, history verification, automatic-evidence summary, and shadow-learning phase—but not raw evidence events, prompts, snippets, or model output.
