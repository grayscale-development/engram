...
The server provides nine tools:
...
8. `engram_shadow_report` returns the local shadow-learning phase, independent-review count, accuracy, and activation state.
9. `engram_shadow_record` records one source-verified verdict for a `focus` observation. Only independent correct/incorrect reviews can promote assisted focus.
...
Every `engram_focus` response includes a `shadow.observation_id` when local shadow learning is available. A distinct reviewer agent can submit a source-verified verdict with `engram_shadow_record`; see [shadow mode](shadow-mode.md) for the promotion policy and privacy boundaries.
