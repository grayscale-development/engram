...
import { doctorReport } from '../src/hosts.js';
...
test('init creates an empty agent-owned Cortex and installs the Cerebellum', async () => {
  ...
  assert.match(skill, /node \.engram\/runtime\/bin\/engram\.js/); assert.doesNotMatch(output, /ask your agent/i);
  ...
  const workflowSkill = await fs.readFile(path.join(root, '.engram/skills/engram-workflow/SKILL.md'), 'utf8'); assert.match(workflowSkill, /Session health check/); assert.match(workflowSkill, /doctor --check-update/); assert.match(workflowSkill, /Silent default/); assert.match(workflowSkill, /🧠 Calibrating Cortex - <accuracy_percent>%/); assert.doesNotMatch(workflowSkill, /Progress to assisted focus/);
  assert.match(workflowSkill, /--no-shadow/); assert.match(workflowSkill, /records `incorrect`/);
  ...
  const agentInstructions = await fs.readFile(path.join(root, 'AGENTS.md'), 'utf8'); assert.match(agentInstructions, /engram:automatic-workflow:start/); assert.match(agentInstructions, /doctor --check-update/); assert.match(agentInstructions, /Cortex reviewer/); assert.match(agentInstructions, /🧠 Calibrating Cortex - <accuracy_percent>%/); assert.match(agentInstructions, /⚠️ Engram check/); assert.doesNotMatch(agentInstructions, /Progress to assisted focus/);
  ...
  assert.deepEqual(JSON.parse(await fs.readFile(path.join(root, '.engram/runtime/package.json'), 'utf8')), { private: true, type: 'module', engram_version: '4.0.2' });
  ...
  const update = await doctorReport(root, { checkUpdate: true, getLatestVersion: async () => '4.0.3' }); assert.equal(update.status, 'update_available'); assert.equal(update.update.status, 'update_available'); assert.match(update.issues[0], /4\.0\.3/);
  await fs.writeFile(path.join(root, '.engram/shadow.json'), '{}'); const invalidSettings = await doctorReport(root); assert.equal(invalidSettings.status, 'needs_setup'); assert.ok(invalidSettings.issues.includes('shadow-settings-valid is missing or invalid'));
  await fs.writeFile(path.join(root, '.engram/shadow.json'), JSON.stringify({ version: 1, enabled: true, activation: 'shadow', target_accuracy_percent: 95, minimum_independent_reviews: 20, promoted_at: null }));
...
test('out-of-scope focus keeps navigation and activity evidence without changing shadow calibration', async () => {
  const root = await fixture(); await run(root, 'init');
  const cli = JSON.parse(await runInstalled(root, 'focus', '--query', 'Engram host configuration', '--no-shadow'));
  assert.equal(cli.shadow.phase, 'excluded'); assert.equal(cli.shadow.observation_id, null); assert.equal(cli.shadow.review_required, false);
  const mcp = await handleMcpRequest({ method: 'tools/call', params: { name: 'engram_focus', arguments: { root, query: 'Engram workflow maintenance', track_shadow: false } } });
  assert.equal(mcp.structuredContent.shadow.phase, 'excluded'); assert.equal(mcp.structuredContent.shadow.observation_id, null); assert.equal(mcp.structuredContent.shadow.review_required, false);
  await assert.rejects(fs.access(path.join(root, '.engram/shadow.ndjson')));
  assert.equal((await automaticEvidenceSummary(root)).focus_actions, 2);
});
...
test('init preserves an existing Cortex and compact reads can include revisions', async () => {
  ...
  assert.match(await run(root, 'init'), /Engram ready\./);
