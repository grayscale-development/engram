...
test('init creates an empty agent-owned Cortex and installs the Cerebellum', async () => {
  ...
  const workflowSkill = await fs.readFile(path.join(root, '.engram/skills/engram-workflow/SKILL.md'), 'utf8'); assert.match(workflowSkill, /Silent default/); assert.match(workflowSkill, /🧠 Calibrating Cortex - <accuracy_percent>%/); assert.doesNotMatch(workflowSkill, /Progress to assisted focus/);
  ...
  const agentInstructions = await fs.readFile(path.join(root, 'AGENTS.md'), 'utf8'); assert.match(agentInstructions, /engram:automatic-workflow:start/); assert.match(agentInstructions, /Cortex reviewer/); assert.match(agentInstructions, /🧠 Calibrating Cortex - <accuracy_percent>%/); assert.doesNotMatch(agentInstructions, /Progress to assisted focus/);
