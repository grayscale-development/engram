...
test('init creates an empty agent-owned Cortex and installs the Cerebellum', async () => {
  ...
  assert.deepEqual(JSON.parse(await fs.readFile(path.join(root, '.engram/runtime/package.json'), 'utf8')), { private: true, type: 'module', engram_version: '4.0.1' });
  ...
  const update = await doctorReport(root, { checkUpdate: true, getLatestVersion: async () => '4.0.2' }); assert.equal(update.status, 'update_available'); assert.equal(update.update.status, 'update_available'); assert.match(update.issues[0], /4\.0\.2/);
